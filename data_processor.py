...
class ComprasProcessor:
    ...
    def procesar_datos(self, df_experto, df_cancelados, df_precompra=None):
        ...
        sesion_id = self._guardar_resultados(len(df_experto), total_procesados, total_en_proceso, eficiencia, cancelados_filtrados, df_procesados, df_en_proceso, COL_NUM_REQ, COL_TIPO_COMPRA, COL_ESTADO_OC)
    ...
    def _guardar_datos_financieros(self, sesion_id, df_analisis_financiero, df_experto_bruto=None):
        ...
        try:
            with sqlite3.connect('compras.db') as conn:
                ...
                mapa_columnas = {
                    'numero_oc': ['N° Orden', 'oc_normalizada'],
    ...
    def procesar_analisis_financiero(self, df_resultado_oc, df_experto_historico, sesion_id=None, tipo_filtro=None):
        ...
        try:
            ...
            COL_NUM_REQ, COL_TIPO_COMPRA, COL_ESTADO_OC = self._detectar_columnas(df_experto_historico)
            ...
            terminos_exclusion = ['cancelad', 'rechazad', 'no aceptad']
            ...
            resultado['kpis'] = {
                'monto_total': f"${monto_total:,.0f}", 'monto_conforme': f"${monto_conforme:,.0f}", 'efectividad': f"{efectividad_recepcion:.1f}%"
            }
